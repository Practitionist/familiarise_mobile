-- ─────────────────────────────────────────────────────────────
-- VENDORED from familiarise_web (which owns the Prisma schema).
-- Do not edit here. To refresh after an upstream change:
--   cp ~/Desktop/familiarise_web/prisma/sql/check-constraints.sql \
--      backend/prisma/sql/check-constraints.sql
--
-- Applied by docker/db-init against the local Postgres. The upstream
-- `-- SPLIT` markers are plain comments here: psql runs the whole file
-- in one go, so no statement splitting is needed (they exist only
-- because Prisma $executeRawUnsafe takes one statement per call).
-- ─────────────────────────────────────────────────────────────

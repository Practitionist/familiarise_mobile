...
class MemberDelegate {
  ...
  MemberDelegate(this._executor);

  /// Find a single Member by unique field(s)
  Future<Member?> findUnique({
    required MemberWhereUniqueInput where,
  }) async {
    ...
    return result != null ? Member.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single Member or throw if not found
  Future<Member> findUniqueOrThrow({
    required MemberWhereUniqueInput where,
  }) async {
  ...
  /// Find the first Member matching criteria
  Future<Member?> findFirst({
    MemberWhereInput? where,
    MemberOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple Members with optional filters
  Future<List<Member>> findMany({
    MemberWhereInput? where,
    MemberOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => Member.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new Member
  Future<Member> create({
    required CreateMemberInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Member')
    ...
    return Member.fromJson(_normalizeForJson(result));
  }

  /// Create multiple Members
  Future<int> createMany({
    required List<CreateMemberInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Member')
  ...
  Future<Member> update({
    required MemberWhereUniqueInput where,
    required UpdateMemberInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Member')
  ...
  Future<int> updateMany({
    required MemberWhereInput where,
    required UpdateMemberInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Member')
  ...
  /// Group Members by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(MemberWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(MemberWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as MemberWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is MemberWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(MemberOrderByInput orderBy) {

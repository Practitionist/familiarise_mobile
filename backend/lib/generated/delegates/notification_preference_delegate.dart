...
class NotificationPreferenceDelegate {
  ...
  NotificationPreferenceDelegate(this._executor);

  /// Find a single NotificationPreference by unique field(s)
  Future<NotificationPreference?> findUnique({
    required NotificationPreferenceWhereUniqueInput where,
  }) async {
    ...
    return result != null ? NotificationPreference.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single NotificationPreference or throw if not found
  Future<NotificationPreference> findUniqueOrThrow({
    required NotificationPreferenceWhereUniqueInput where,
  }) async {
    final result = await findUnique(where: where);
    if (result == null) {
      throw Exception('NotificationPreference not found');
    }
    return result;
  }

  /// Find the first NotificationPreference matching criteria
  Future<NotificationPreference?> findFirst({
    NotificationPreferenceWhereInput? where,
    NotificationPreferenceOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple NotificationPreferences with optional filters
  Future<List<NotificationPreference>> findMany({
    NotificationPreferenceWhereInput? where,
    NotificationPreferenceOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => NotificationPreference.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new NotificationPreference
  Future<NotificationPreference> create({
    required CreateNotificationPreferenceInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('NotificationPreference')
    ...
    final result = await _executor.executeQueryAsSingleMap(query);
    if (result == null) {
      throw Exception('Failed to create NotificationPreference');
    }
    return NotificationPreference.fromJson(_normalizeForJson(result));
  }

  /// Create multiple NotificationPreferences
  Future<int> createMany({
    required List<CreateNotificationPreferenceInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('NotificationPreference')
  ...
  Future<NotificationPreference> update({
    required NotificationPreferenceWhereUniqueInput where,
    required UpdateNotificationPreferenceInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('NotificationPreference')
  ...
  /// Update multiple NotificationPreferences
  Future<int> updateMany({
    required NotificationPreferenceWhereInput where,
    required UpdateNotificationPreferenceInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('NotificationPreference')
  ...
  /// Count NotificationPreferences matching criteria
  Future<int> count({
    NotificationPreferenceWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group NotificationPreferences by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(NotificationPreferenceWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(NotificationPreferenceWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as NotificationPreferenceWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is NotificationPreferenceWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(NotificationPreferenceOrderByInput orderBy) {

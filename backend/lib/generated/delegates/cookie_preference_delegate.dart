...
class CookiePreferenceDelegate {
  ...
  CookiePreferenceDelegate(this._executor);

  /// Find a single CookiePreference by unique field(s)
  Future<CookiePreference?> findUnique({
    required CookiePreferenceWhereUniqueInput where,
  }) async {
    ...
    return result != null ? CookiePreference.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single CookiePreference or throw if not found
  Future<CookiePreference> findUniqueOrThrow({
    required CookiePreferenceWhereUniqueInput where,
  }) async {
    final result = await findUnique(where: where);
    if (result == null) {
      throw Exception('CookiePreference not found');
    }
    return result;
  }

  /// Find the first CookiePreference matching criteria
  Future<CookiePreference?> findFirst({
    CookiePreferenceWhereInput? where,
    CookiePreferenceOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple CookiePreferences with optional filters
  Future<List<CookiePreference>> findMany({
    CookiePreferenceWhereInput? where,
    CookiePreferenceOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => CookiePreference.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new CookiePreference
  Future<CookiePreference> create({
    required CreateCookiePreferenceInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('CookiePreference')
    ...
    final result = await _executor.executeQueryAsSingleMap(query);
    if (result == null) {
      throw Exception('Failed to create CookiePreference');
    }
    return CookiePreference.fromJson(_normalizeForJson(result));
  }

  /// Create multiple CookiePreferences
  Future<int> createMany({
    required List<CreateCookiePreferenceInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('CookiePreference')
  ...
  /// Update a CookiePreference
  Future<CookiePreference> update({
    required CookiePreferenceWhereUniqueInput where,
    required UpdateCookiePreferenceInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('CookiePreference')
  ...
  /// Update multiple CookiePreferences
  Future<int> updateMany({
    required CookiePreferenceWhereInput where,
    required UpdateCookiePreferenceInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('CookiePreference')
  ...
  /// Delete a CookiePreference
  Future<CookiePreference> delete({
    required CookiePreferenceWhereUniqueInput where,
  }) async {
  ...
  /// Delete multiple CookiePreferences
  ...
  /// Count CookiePreferences matching criteria
  Future<int> count({
    CookiePreferenceWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group CookiePreferences by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(CookiePreferenceWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(CookiePreferenceWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as CookiePreferenceWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is CookiePreferenceWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(CookiePreferenceOrderByInput orderBy) {

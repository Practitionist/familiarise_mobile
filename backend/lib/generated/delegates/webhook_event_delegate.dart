...
class WebhookEventDelegate {
  final BaseExecutor _executor;

  WebhookEventDelegate(this._executor);

  /// Find a single WebhookEvent by unique field(s)
  Future<WebhookEvent?> findUnique({
    required WebhookEventWhereUniqueInput where,
  }) async {
    ...
    return result != null ? WebhookEvent.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single WebhookEvent or throw if not found
  Future<WebhookEvent> findUniqueOrThrow({
    required WebhookEventWhereUniqueInput where,
  }) async {
    final result = await findUnique(where: where);
    if (result == null) {
      throw Exception('WebhookEvent not found');
    }
    return result;
  }

  /// Find the first WebhookEvent matching criteria
  Future<WebhookEvent?> findFirst({
    WebhookEventWhereInput? where,
    WebhookEventOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple WebhookEvents with optional filters
  Future<List<WebhookEvent>> findMany({
    WebhookEventWhereInput? where,
    WebhookEventOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => WebhookEvent.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new WebhookEvent
  Future<WebhookEvent> create({
    required CreateWebhookEventInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('WebhookEvent')
    ...
    final result = await _executor.executeQueryAsSingleMap(query);
    if (result == null) {
      throw Exception('Failed to create WebhookEvent');
    }
    return WebhookEvent.fromJson(_normalizeForJson(result));
  }

  /// Create multiple WebhookEvents
  Future<int> createMany({
    required List<CreateWebhookEventInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('WebhookEvent')
  ...
  /// Update a WebhookEvent
  Future<WebhookEvent> update({
    required WebhookEventWhereUniqueInput where,
    required UpdateWebhookEventInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('WebhookEvent')
  ...
  /// Update multiple WebhookEvents
  Future<int> updateMany({
    required WebhookEventWhereInput where,
    required UpdateWebhookEventInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('WebhookEvent')
  ...
  /// Count WebhookEvents matching criteria
  Future<int> count({
    WebhookEventWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group WebhookEvents by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(WebhookEventWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(WebhookEventWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as WebhookEventWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is WebhookEventWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(WebhookEventOrderByInput orderBy) {

...
class ConsultantTaxInfoDelegate {
  final BaseExecutor _executor;

  ConsultantTaxInfoDelegate(this._executor);

  /// Find a single ConsultantTaxInfo by unique field(s)
  Future<ConsultantTaxInfo?> findUnique({
    required ConsultantTaxInfoWhereUniqueInput where,
  }) async {
    ...
    return result != null ? ConsultantTaxInfo.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single ConsultantTaxInfo or throw if not found
  Future<ConsultantTaxInfo> findUniqueOrThrow({
    required ConsultantTaxInfoWhereUniqueInput where,
  }) async {
    final result = await findUnique(where: where);
    if (result == null) {
      throw Exception('ConsultantTaxInfo not found');
    }
    return result;
  }

  /// Find the first ConsultantTaxInfo matching criteria
  Future<ConsultantTaxInfo?> findFirst({
    ConsultantTaxInfoWhereInput? where,
    ConsultantTaxInfoOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple ConsultantTaxInfos with optional filters
  Future<List<ConsultantTaxInfo>> findMany({
    ConsultantTaxInfoWhereInput? where,
    ConsultantTaxInfoOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => ConsultantTaxInfo.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new ConsultantTaxInfo
  Future<ConsultantTaxInfo> create({
    required CreateConsultantTaxInfoInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('ConsultantTaxInfo')
    ...
    final result = await _executor.executeQueryAsSingleMap(query);
    if (result == null) {
      throw Exception('Failed to create ConsultantTaxInfo');
    }
    return ConsultantTaxInfo.fromJson(_normalizeForJson(result));
  }

  /// Create multiple ConsultantTaxInfos
  Future<int> createMany({
    required List<CreateConsultantTaxInfoInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('ConsultantTaxInfo')
  ...
  /// Update a ConsultantTaxInfo
  Future<ConsultantTaxInfo> update({
    required ConsultantTaxInfoWhereUniqueInput where,
    required UpdateConsultantTaxInfoInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('ConsultantTaxInfo')
  ...
  /// Update multiple ConsultantTaxInfos
  Future<int> updateMany({
    required ConsultantTaxInfoWhereInput where,
    required UpdateConsultantTaxInfoInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('ConsultantTaxInfo')
  ...
  /// Delete a ConsultantTaxInfo
  Future<ConsultantTaxInfo> delete({
    required ConsultantTaxInfoWhereUniqueInput where,
  }) async {
  ...
  /// Delete multiple ConsultantTaxInfos
  ...
  /// Count ConsultantTaxInfos matching criteria
  Future<int> count({
    ConsultantTaxInfoWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group ConsultantTaxInfos by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(ConsultantTaxInfoWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(ConsultantTaxInfoWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as ConsultantTaxInfoWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is ConsultantTaxInfoWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(ConsultantTaxInfoOrderByInput orderBy) {

...
class PayoutAccountDelegate {
  final BaseExecutor _executor;

  PayoutAccountDelegate(this._executor);

  /// Find a single PayoutAccount by unique field(s)
  Future<PayoutAccount?> findUnique({
    required PayoutAccountWhereUniqueInput where,
  }) async {
    ...
    return result != null ? PayoutAccount.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single PayoutAccount or throw if not found
  Future<PayoutAccount> findUniqueOrThrow({
    required PayoutAccountWhereUniqueInput where,
  }) async {
    final result = await findUnique(where: where);
    if (result == null) {
      throw Exception('PayoutAccount not found');
    }
    return result;
  }

  /// Find the first PayoutAccount matching criteria
  Future<PayoutAccount?> findFirst({
    PayoutAccountWhereInput? where,
    PayoutAccountOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple PayoutAccounts with optional filters
  Future<List<PayoutAccount>> findMany({
    PayoutAccountWhereInput? where,
    PayoutAccountOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => PayoutAccount.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new PayoutAccount
  Future<PayoutAccount> create({
    required CreatePayoutAccountInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('PayoutAccount')
    ...
    final result = await _executor.executeQueryAsSingleMap(query);
    if (result == null) {
      throw Exception('Failed to create PayoutAccount');
    }
    return PayoutAccount.fromJson(_normalizeForJson(result));
  }

  /// Create multiple PayoutAccounts
  Future<int> createMany({
    required List<CreatePayoutAccountInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('PayoutAccount')
  ...
  /// Update a PayoutAccount
  Future<PayoutAccount> update({
    required PayoutAccountWhereUniqueInput where,
    required UpdatePayoutAccountInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('PayoutAccount')
  ...
  /// Update multiple PayoutAccounts
  Future<int> updateMany({
    required PayoutAccountWhereInput where,
    required UpdatePayoutAccountInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('PayoutAccount')
  ...
  /// Delete multiple PayoutAccounts
  ...
  /// Count PayoutAccounts matching criteria
  Future<int> count({
    PayoutAccountWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group PayoutAccounts by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(PayoutAccountWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(PayoutAccountWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as PayoutAccountWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is PayoutAccountWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(PayoutAccountOrderByInput orderBy) {

...
class InvitationDelegate {
  final BaseExecutor _executor;

  InvitationDelegate(this._executor);

  /// Find a single Invitation by unique field(s)
  Future<Invitation?> findUnique({
    required InvitationWhereUniqueInput where,
  }) async {
    ...
    return result != null ? Invitation.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single Invitation or throw if not found
  Future<Invitation> findUniqueOrThrow({
    required InvitationWhereUniqueInput where,
  }) async {
  ...
  /// Find the first Invitation matching criteria
  Future<Invitation?> findFirst({
    InvitationWhereInput? where,
    InvitationOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple Invitations with optional filters
  Future<List<Invitation>> findMany({
    InvitationWhereInput? where,
    InvitationOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => Invitation.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new Invitation
  Future<Invitation> create({
    required CreateInvitationInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Invitation')
    ...
    return Invitation.fromJson(_normalizeForJson(result));
  }

  /// Create multiple Invitations
  Future<int> createMany({
    required List<CreateInvitationInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Invitation')
  ...
  Future<Invitation> update({
    required InvitationWhereUniqueInput where,
    required UpdateInvitationInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Invitation')
  ...
  Future<int> updateMany({
    required InvitationWhereInput where,
    required UpdateInvitationInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Invitation')
  ...
  /// Count Invitations matching criteria
  Future<int> count({
    InvitationWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group Invitations by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(InvitationWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(InvitationWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as InvitationWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is InvitationWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(InvitationOrderByInput orderBy) {

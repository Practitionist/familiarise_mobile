...
class ClassModelDelegate {
  final BaseExecutor _executor;

  ClassModelDelegate(this._executor);

  /// Find a single ClassModel by unique field(s)
  Future<ClassModel?> findUnique({
    required ClassModelWhereUniqueInput where,
  }) async {
    ...
    return result != null ? ClassModel.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single ClassModel or throw if not found
  Future<ClassModel> findUniqueOrThrow({
    required ClassModelWhereUniqueInput where,
  }) async {
    final result = await findUnique(where: where);
    if (result == null) {
      throw Exception('ClassModel not found');
    }
    return result;
  }

  /// Find the first ClassModel matching criteria
  Future<ClassModel?> findFirst({
    ClassModelWhereInput? where,
    ClassModelOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple ClassModels with optional filters
  Future<List<ClassModel>> findMany({
    ClassModelWhereInput? where,
    ClassModelOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => ClassModel.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new ClassModel
  Future<ClassModel> create({
    required CreateClassModelInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Class')
    ...
    final result = await _executor.executeQueryAsSingleMap(query);
    if (result == null) {
      throw Exception('Failed to create ClassModel');
    }
    return ClassModel.fromJson(_normalizeForJson(result));
  }

  /// Create multiple ClassModels
  Future<int> createMany({
    required List<CreateClassModelInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Class')
  ...
  Future<ClassModel> update({
    required ClassModelWhereUniqueInput where,
    required UpdateClassModelInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Class')
  ...
  /// Update multiple ClassModels
  Future<int> updateMany({
    required ClassModelWhereInput where,
    required UpdateClassModelInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Class')
  ...
  /// Count ClassModels matching criteria
  Future<int> count({
    ClassModelWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group ClassModels by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(ClassModelWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(ClassModelWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as ClassModelWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is ClassModelWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(ClassModelOrderByInput orderBy) {

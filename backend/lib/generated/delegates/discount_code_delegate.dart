...
class DiscountCodeDelegate {
  ...
  DiscountCodeDelegate(this._executor);

  /// Find a single DiscountCode by unique field(s)
  Future<DiscountCode?> findUnique({
    required DiscountCodeWhereUniqueInput where,
  }) async {
    ...
    return result != null ? DiscountCode.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single DiscountCode or throw if not found
  Future<DiscountCode> findUniqueOrThrow({
    required DiscountCodeWhereUniqueInput where,
  }) async {
    final result = await findUnique(where: where);
    if (result == null) {
      throw Exception('DiscountCode not found');
    }
    return result;
  }

  /// Find the first DiscountCode matching criteria
  Future<DiscountCode?> findFirst({
    DiscountCodeWhereInput? where,
    DiscountCodeOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple DiscountCodes with optional filters
  Future<List<DiscountCode>> findMany({
    DiscountCodeWhereInput? where,
    DiscountCodeOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => DiscountCode.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new DiscountCode
  Future<DiscountCode> create({
    required CreateDiscountCodeInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('DiscountCode')
    ...
    final result = await _executor.executeQueryAsSingleMap(query);
    if (result == null) {
      throw Exception('Failed to create DiscountCode');
    }
    return DiscountCode.fromJson(_normalizeForJson(result));
  }

  /// Create multiple DiscountCodes
  Future<int> createMany({
    required List<CreateDiscountCodeInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('DiscountCode')
  ...
  Future<DiscountCode> update({
    required DiscountCodeWhereUniqueInput where,
    required UpdateDiscountCodeInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('DiscountCode')
  ...
  /// Update multiple DiscountCodes
  Future<int> updateMany({
    required DiscountCodeWhereInput where,
    required UpdateDiscountCodeInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('DiscountCode')
  ...
  /// Count DiscountCodes matching criteria
  Future<int> count({
    DiscountCodeWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group DiscountCodes by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(DiscountCodeWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(DiscountCodeWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as DiscountCodeWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is DiscountCodeWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(DiscountCodeOrderByInput orderBy) {

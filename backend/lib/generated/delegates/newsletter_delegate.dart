...
class NewsletterDelegate {
  ...
  NewsletterDelegate(this._executor);

  /// Find a single Newsletter by unique field(s)
  Future<Newsletter?> findUnique({
    required NewsletterWhereUniqueInput where,
  }) async {
    ...
    return result != null ? Newsletter.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single Newsletter or throw if not found
  Future<Newsletter> findUniqueOrThrow({
    required NewsletterWhereUniqueInput where,
  }) async {
    final result = await findUnique(where: where);
    if (result == null) {
      throw Exception('Newsletter not found');
    }
    return result;
  }

  /// Find the first Newsletter matching criteria
  Future<Newsletter?> findFirst({
    NewsletterWhereInput? where,
    NewsletterOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple Newsletters with optional filters
  Future<List<Newsletter>> findMany({
    NewsletterWhereInput? where,
    NewsletterOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => Newsletter.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new Newsletter
  Future<Newsletter> create({
    required CreateNewsletterInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Newsletter')
    ...
    final result = await _executor.executeQueryAsSingleMap(query);
    if (result == null) {
      throw Exception('Failed to create Newsletter');
    }
    return Newsletter.fromJson(_normalizeForJson(result));
  }

  /// Create multiple Newsletters
  Future<int> createMany({
    required List<CreateNewsletterInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Newsletter')
  ...
  /// Update a Newsletter
  Future<Newsletter> update({
    required NewsletterWhereUniqueInput where,
    required UpdateNewsletterInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Newsletter')
  ...
  Future<int> updateMany({
    required NewsletterWhereInput where,
    required UpdateNewsletterInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Newsletter')
  ...
  /// Count Newsletters matching criteria
  Future<int> count({
    NewsletterWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group Newsletters by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(NewsletterWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(NewsletterWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as NewsletterWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is NewsletterWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(NewsletterOrderByInput orderBy) {

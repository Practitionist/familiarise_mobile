...
class SlotOfAvailabilityCustomDelegate {
  ...
  SlotOfAvailabilityCustomDelegate(this._executor);

  /// Find a single SlotOfAvailabilityCustom by unique field(s)
  Future<SlotOfAvailabilityCustom?> findUnique({
    required SlotOfAvailabilityCustomWhereUniqueInput where,
  }) async {
    ...
    return result != null ? SlotOfAvailabilityCustom.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single SlotOfAvailabilityCustom or throw if not found
  Future<SlotOfAvailabilityCustom> findUniqueOrThrow({
    required SlotOfAvailabilityCustomWhereUniqueInput where,
  }) async {
    final result = await findUnique(where: where);
    if (result == null) {
      throw Exception('SlotOfAvailabilityCustom not found');
    }
    return result;
  }

  /// Find the first SlotOfAvailabilityCustom matching criteria
  Future<SlotOfAvailabilityCustom?> findFirst({
    SlotOfAvailabilityCustomWhereInput? where,
    SlotOfAvailabilityCustomOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple SlotOfAvailabilityCustoms with optional filters
  Future<List<SlotOfAvailabilityCustom>> findMany({
    SlotOfAvailabilityCustomWhereInput? where,
    SlotOfAvailabilityCustomOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => SlotOfAvailabilityCustom.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new SlotOfAvailabilityCustom
  Future<SlotOfAvailabilityCustom> create({
    required CreateSlotOfAvailabilityCustomInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('SlotOfAvailabilityCustom')
    ...
    final result = await _executor.executeQueryAsSingleMap(query);
    if (result == null) {
      throw Exception('Failed to create SlotOfAvailabilityCustom');
    }
    return SlotOfAvailabilityCustom.fromJson(_normalizeForJson(result));
  }

  /// Create multiple SlotOfAvailabilityCustoms
  Future<int> createMany({
    required List<CreateSlotOfAvailabilityCustomInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('SlotOfAvailabilityCustom')
  ...
  /// Update a SlotOfAvailabilityCustom
  Future<SlotOfAvailabilityCustom> update({
    required SlotOfAvailabilityCustomWhereUniqueInput where,
    required UpdateSlotOfAvailabilityCustomInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('SlotOfAvailabilityCustom')
  ...
  /// Update multiple SlotOfAvailabilityCustoms
  Future<int> updateMany({
    required SlotOfAvailabilityCustomWhereInput where,
    required UpdateSlotOfAvailabilityCustomInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('SlotOfAvailabilityCustom')
  ...
  /// Delete a SlotOfAvailabilityCustom
  Future<SlotOfAvailabilityCustom> delete({
    required SlotOfAvailabilityCustomWhereUniqueInput where,
  }) async {
  ...
  /// Delete multiple SlotOfAvailabilityCustoms
  ...
  /// Count SlotOfAvailabilityCustoms matching criteria
  Future<int> count({
    SlotOfAvailabilityCustomWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group SlotOfAvailabilityCustoms by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(SlotOfAvailabilityCustomWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(SlotOfAvailabilityCustomWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as SlotOfAvailabilityCustomWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is SlotOfAvailabilityCustomWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(SlotOfAvailabilityCustomOrderByInput orderBy) {

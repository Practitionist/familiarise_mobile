...
class InvoiceDelegate {
  ...
  InvoiceDelegate(this._executor);

  /// Find a single Invoice by unique field(s)
  Future<Invoice?> findUnique({
    required InvoiceWhereUniqueInput where,
  }) async {
    ...
    return result != null ? Invoice.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single Invoice or throw if not found
  Future<Invoice> findUniqueOrThrow({
    required InvoiceWhereUniqueInput where,
  }) async {
  ...
  /// Find the first Invoice matching criteria
  Future<Invoice?> findFirst({
    InvoiceWhereInput? where,
    InvoiceOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple Invoices with optional filters
  Future<List<Invoice>> findMany({
    InvoiceWhereInput? where,
    InvoiceOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => Invoice.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new Invoice
  Future<Invoice> create({
    required CreateInvoiceInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Invoice')
    ...
    return Invoice.fromJson(_normalizeForJson(result));
  }

  /// Create multiple Invoices
  Future<int> createMany({
    required List<CreateInvoiceInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Invoice')
  ...
  Future<Invoice> update({
    required InvoiceWhereUniqueInput where,
    required UpdateInvoiceInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Invoice')
  ...
  Future<int> updateMany({
    required InvoiceWhereInput where,
    required UpdateInvoiceInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Invoice')
  ...
  /// Group Invoices by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(InvoiceWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(InvoiceWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as InvoiceWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is InvoiceWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(InvoiceOrderByInput orderBy) {

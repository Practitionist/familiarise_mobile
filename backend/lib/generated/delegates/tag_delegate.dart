...
class TagDelegate {
  ...
  TagDelegate(this._executor);

  /// Find a single Tag by unique field(s)
  Future<Tag?> findUnique({
    ...
    return result != null ? Tag.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single Tag or throw if not found
  Future<Tag> findUniqueOrThrow({
    required TagWhereUniqueInput where,
  }) async {
  ...
  /// Find the first Tag matching criteria
  Future<Tag?> findFirst({
    TagWhereInput? where,
    TagOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple Tags with optional filters
  Future<List<Tag>> findMany({
    TagWhereInput? where,
    TagOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => Tag.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new Tag
  Future<Tag> create({
    required CreateTagInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Tag')
    ...
    return Tag.fromJson(_normalizeForJson(result));
  }

  /// Create multiple Tags
  Future<int> createMany({
    required List<CreateTagInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Tag')
  ...
  /// Group Tags by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(TagWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(TagWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as TagWhereInput).toJson();
          }).toList();
        }
  ...
  Map<String, dynamic> _orderByToJson(TagOrderByInput orderBy) {

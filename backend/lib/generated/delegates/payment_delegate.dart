...
class PaymentDelegate {
  ...
  PaymentDelegate(this._executor);

  /// Find a single Payment by unique field(s)
  Future<Payment?> findUnique({
    required PaymentWhereUniqueInput where,
  }) async {
    ...
    return result != null ? Payment.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single Payment or throw if not found
  Future<Payment> findUniqueOrThrow({
    required PaymentWhereUniqueInput where,
  }) async {
  ...
  /// Find the first Payment matching criteria
  Future<Payment?> findFirst({
    PaymentWhereInput? where,
    PaymentOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple Payments with optional filters
  Future<List<Payment>> findMany({
    PaymentWhereInput? where,
    PaymentOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => Payment.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new Payment
  Future<Payment> create({
    required CreatePaymentInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Payment')
    ...
    return Payment.fromJson(_normalizeForJson(result));
  }

  /// Create multiple Payments
  Future<int> createMany({
    required List<CreatePaymentInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Payment')
  ...
  Future<Payment> update({
    required PaymentWhereUniqueInput where,
    required UpdatePaymentInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Payment')
  ...
  Future<int> updateMany({
    required PaymentWhereInput where,
    required UpdatePaymentInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Payment')
  ...
  /// Group Payments by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(PaymentWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(PaymentWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as PaymentWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is PaymentWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(PaymentOrderByInput orderBy) {

...
class StaffProfileDelegate {
  ...
  StaffProfileDelegate(this._executor);

  /// Find a single StaffProfile by unique field(s)
  Future<StaffProfile?> findUnique({
    required StaffProfileWhereUniqueInput where,
  }) async {
    ...
    return result != null ? StaffProfile.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single StaffProfile or throw if not found
  Future<StaffProfile> findUniqueOrThrow({
    required StaffProfileWhereUniqueInput where,
  }) async {
    final result = await findUnique(where: where);
    if (result == null) {
      throw Exception('StaffProfile not found');
    }
    return result;
  }

  /// Find the first StaffProfile matching criteria
  Future<StaffProfile?> findFirst({
    StaffProfileWhereInput? where,
    StaffProfileOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple StaffProfiles with optional filters
  Future<List<StaffProfile>> findMany({
    StaffProfileWhereInput? where,
    StaffProfileOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => StaffProfile.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new StaffProfile
  Future<StaffProfile> create({
    required CreateStaffProfileInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('StaffProfile')
    ...
    final result = await _executor.executeQueryAsSingleMap(query);
    if (result == null) {
      throw Exception('Failed to create StaffProfile');
    }
    return StaffProfile.fromJson(_normalizeForJson(result));
  }

  /// Create multiple StaffProfiles
  Future<int> createMany({
    required List<CreateStaffProfileInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('StaffProfile')
  ...
  Future<StaffProfile> update({
    required StaffProfileWhereUniqueInput where,
    required UpdateStaffProfileInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('StaffProfile')
  ...
  /// Update multiple StaffProfiles
  Future<int> updateMany({
    required StaffProfileWhereInput where,
    required UpdateStaffProfileInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('StaffProfile')
  ...
  /// Count StaffProfiles matching criteria
  Future<int> count({
    StaffProfileWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group StaffProfiles by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(StaffProfileWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(StaffProfileWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as StaffProfileWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is StaffProfileWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(StaffProfileOrderByInput orderBy) {

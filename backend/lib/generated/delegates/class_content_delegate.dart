...
class ClassContentDelegate {
  ...
  ClassContentDelegate(this._executor);

  /// Find a single ClassContent by unique field(s)
  Future<ClassContent?> findUnique({
    required ClassContentWhereUniqueInput where,
  }) async {
    ...
    return result != null ? ClassContent.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single ClassContent or throw if not found
  Future<ClassContent> findUniqueOrThrow({
    required ClassContentWhereUniqueInput where,
  }) async {
    final result = await findUnique(where: where);
    if (result == null) {
      throw Exception('ClassContent not found');
    }
    return result;
  }

  /// Find the first ClassContent matching criteria
  Future<ClassContent?> findFirst({
    ClassContentWhereInput? where,
    ClassContentOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple ClassContents with optional filters
  Future<List<ClassContent>> findMany({
    ClassContentWhereInput? where,
    ClassContentOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => ClassContent.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new ClassContent
  Future<ClassContent> create({
    required CreateClassContentInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('ClassContent')
    ...
    final result = await _executor.executeQueryAsSingleMap(query);
    if (result == null) {
      throw Exception('Failed to create ClassContent');
    }
    return ClassContent.fromJson(_normalizeForJson(result));
  }

  /// Create multiple ClassContents
  Future<int> createMany({
    required List<CreateClassContentInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('ClassContent')
  ...
  /// Update a ClassContent
  Future<ClassContent> update({
    required ClassContentWhereUniqueInput where,
    required UpdateClassContentInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('ClassContent')
  ...
  /// Update multiple ClassContents
  Future<int> updateMany({
    required ClassContentWhereInput where,
    required UpdateClassContentInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('ClassContent')
  ...
  /// Count ClassContents matching criteria
  Future<int> count({
    ClassContentWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group ClassContents by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(ClassContentWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(ClassContentWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as ClassContentWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is ClassContentWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(ClassContentOrderByInput orderBy) {

...
class ReferralDelegate {
  ...
  ReferralDelegate(this._executor);

  /// Find a single Referral by unique field(s)
  Future<Referral?> findUnique({
    required ReferralWhereUniqueInput where,
  }) async {
    ...
    return result != null ? Referral.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single Referral or throw if not found
  Future<Referral> findUniqueOrThrow({
    required ReferralWhereUniqueInput where,
  }) async {
  ...
  /// Find the first Referral matching criteria
  Future<Referral?> findFirst({
    ReferralWhereInput? where,
    ReferralOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple Referrals with optional filters
  Future<List<Referral>> findMany({
    ReferralWhereInput? where,
    ReferralOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => Referral.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new Referral
  Future<Referral> create({
    required CreateReferralInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Referral')
    ...
    return Referral.fromJson(_normalizeForJson(result));
  }

  /// Create multiple Referrals
  Future<int> createMany({
    required List<CreateReferralInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Referral')
  ...
  /// Update a Referral
  Future<Referral> update({
    required ReferralWhereUniqueInput where,
    required UpdateReferralInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Referral')
  ...
  Future<int> updateMany({
    required ReferralWhereInput where,
    required UpdateReferralInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Referral')
  ...
  /// Count Referrals matching criteria
  Future<int> count({
    ReferralWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group Referrals by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(ReferralWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(ReferralWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as ReferralWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is ReferralWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(ReferralOrderByInput orderBy) {

...
class PayoutDelegate {
  ...
  PayoutDelegate(this._executor);

  /// Find a single Payout by unique field(s)
  Future<Payout?> findUnique({
    required PayoutWhereUniqueInput where,
  }) async {
    ...
    return result != null ? Payout.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single Payout or throw if not found
  Future<Payout> findUniqueOrThrow({
    required PayoutWhereUniqueInput where,
  }) async {
  ...
  /// Find the first Payout matching criteria
  Future<Payout?> findFirst({
    PayoutWhereInput? where,
    PayoutOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple Payouts with optional filters
  Future<List<Payout>> findMany({
    PayoutWhereInput? where,
    PayoutOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => Payout.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new Payout
  Future<Payout> create({
    required CreatePayoutInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Payout')
    ...
    return Payout.fromJson(_normalizeForJson(result));
  }

  /// Create multiple Payouts
  Future<int> createMany({
    required List<CreatePayoutInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Payout')
  ...
  Future<Payout> update({
    required PayoutWhereUniqueInput where,
    required UpdatePayoutInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Payout')
  ...
  Future<int> updateMany({
    required PayoutWhereInput where,
    required UpdatePayoutInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Payout')
  ...
  /// Group Payouts by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(PayoutWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(PayoutWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as PayoutWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is PayoutWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(PayoutOrderByInput orderBy) {

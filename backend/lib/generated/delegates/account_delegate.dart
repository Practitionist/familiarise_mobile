...
class AccountDelegate {
  ...
  AccountDelegate(this._executor);

  /// Find a single Account by unique field(s)
  Future<Account?> findUnique({
    ...
    return result != null ? Account.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single Account or throw if not found
  Future<Account> findUniqueOrThrow({
    required AccountWhereUniqueInput where,
  }) async {
  ...
  /// Find the first Account matching criteria
  Future<Account?> findFirst({
    AccountWhereInput? where,
    AccountOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple Accounts with optional filters
  Future<List<Account>> findMany({
    ...
    return results.map((json) => Account.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new Account
  Future<Account> create({
    required CreateAccountInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Account')
    ...
    return Account.fromJson(_normalizeForJson(result));
  }

  /// Create multiple Accounts
  Future<int> createMany({
    required List<CreateAccountInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Account')
  ...
  Future<Account> update({
    required AccountWhereUniqueInput where,
    required UpdateAccountInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Account')
  ...
  /// Count Accounts matching criteria
  Future<int> count({
    AccountWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group Accounts by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(AccountWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(AccountWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as AccountWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is AccountWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(AccountOrderByInput orderBy) {

...
class RefundDelegate {
  ...
  RefundDelegate(this._executor);

  /// Find a single Refund by unique field(s)
  Future<Refund?> findUnique({
    required RefundWhereUniqueInput where,
  }) async {
    ...
    return result != null ? Refund.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single Refund or throw if not found
  Future<Refund> findUniqueOrThrow({
    required RefundWhereUniqueInput where,
  }) async {
  ...
  /// Find the first Refund matching criteria
  Future<Refund?> findFirst({
    RefundWhereInput? where,
    RefundOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple Refunds with optional filters
  Future<List<Refund>> findMany({
    RefundWhereInput? where,
    RefundOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => Refund.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new Refund
  Future<Refund> create({
    required CreateRefundInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Refund')
    ...
    return Refund.fromJson(_normalizeForJson(result));
  }

  /// Create multiple Refunds
  Future<int> createMany({
    required List<CreateRefundInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Refund')
  ...
  Future<Refund> update({
    required RefundWhereUniqueInput where,
    required UpdateRefundInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Refund')
  ...
  /// Update multiple Refunds
  Future<int> updateMany({
    required RefundWhereInput where,
    required UpdateRefundInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('Refund')
  ...
  /// Group Refunds by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(RefundWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(RefundWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as RefundWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is RefundWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(RefundOrderByInput orderBy) {

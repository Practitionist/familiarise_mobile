...
class ModerationReportDelegate {
  final BaseExecutor _executor;

  ModerationReportDelegate(this._executor);

  /// Find a single ModerationReport by unique field(s)
  Future<ModerationReport?> findUnique({
    required ModerationReportWhereUniqueInput where,
  }) async {
    ...
    return result != null ? ModerationReport.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single ModerationReport or throw if not found
  Future<ModerationReport> findUniqueOrThrow({
    required ModerationReportWhereUniqueInput where,
  }) async {
    final result = await findUnique(where: where);
    if (result == null) {
      throw Exception('ModerationReport not found');
    }
    return result;
  }

  /// Find the first ModerationReport matching criteria
  Future<ModerationReport?> findFirst({
    ModerationReportWhereInput? where,
    ModerationReportOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple ModerationReports with optional filters
  Future<List<ModerationReport>> findMany({
    ModerationReportWhereInput? where,
    ModerationReportOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => ModerationReport.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new ModerationReport
  Future<ModerationReport> create({
    required CreateModerationReportInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('ModerationReport')
    ...
    final result = await _executor.executeQueryAsSingleMap(query);
    if (result == null) {
      throw Exception('Failed to create ModerationReport');
    }
    return ModerationReport.fromJson(_normalizeForJson(result));
  }

  /// Create multiple ModerationReports
  Future<int> createMany({
    required List<CreateModerationReportInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('ModerationReport')
  ...
  /// Update a ModerationReport
  Future<ModerationReport> update({
    required ModerationReportWhereUniqueInput where,
    required UpdateModerationReportInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('ModerationReport')
  ...
  /// Update multiple ModerationReports
  Future<int> updateMany({
    required ModerationReportWhereInput where,
    required UpdateModerationReportInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('ModerationReport')
  ...
  Future<ModerationReport> delete({
    required ModerationReportWhereUniqueInput where,
  }) async {
  ...
  /// Delete multiple ModerationReports
  ...
  /// Count ModerationReports matching criteria
  Future<int> count({
    ModerationReportWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group ModerationReports by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(ModerationReportWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(ModerationReportWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as ModerationReportWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is ModerationReportWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(ModerationReportOrderByInput orderBy) {

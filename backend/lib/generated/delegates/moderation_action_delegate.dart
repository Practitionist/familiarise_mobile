...
class ModerationActionDelegate {
  final BaseExecutor _executor;

  ModerationActionDelegate(this._executor);

  /// Find a single ModerationAction by unique field(s)
  Future<ModerationAction?> findUnique({
    required ModerationActionWhereUniqueInput where,
  }) async {
    ...
    return result != null ? ModerationAction.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single ModerationAction or throw if not found
  Future<ModerationAction> findUniqueOrThrow({
    required ModerationActionWhereUniqueInput where,
  }) async {
    final result = await findUnique(where: where);
    if (result == null) {
      throw Exception('ModerationAction not found');
    }
    return result;
  }

  /// Find the first ModerationAction matching criteria
  Future<ModerationAction?> findFirst({
    ModerationActionWhereInput? where,
    ModerationActionOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple ModerationActions with optional filters
  Future<List<ModerationAction>> findMany({
    ModerationActionWhereInput? where,
    ModerationActionOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => ModerationAction.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new ModerationAction
  Future<ModerationAction> create({
    required CreateModerationActionInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('ModerationAction')
    ...
    final result = await _executor.executeQueryAsSingleMap(query);
    if (result == null) {
      throw Exception('Failed to create ModerationAction');
    }
    return ModerationAction.fromJson(_normalizeForJson(result));
  }

  /// Create multiple ModerationActions
  Future<int> createMany({
    required List<CreateModerationActionInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('ModerationAction')
  ...
  Future<ModerationAction> update({
    required ModerationActionWhereUniqueInput where,
    required UpdateModerationActionInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('ModerationAction')
  ...
  /// Update multiple ModerationActions
  Future<int> updateMany({
    required ModerationActionWhereInput where,
    required UpdateModerationActionInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('ModerationAction')
  ...
  Future<ModerationAction> delete({
    required ModerationActionWhereUniqueInput where,
  }) async {
  ...
  /// Delete multiple ModerationActions
  ...
  /// Count ModerationActions matching criteria
  Future<int> count({
    ModerationActionWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group ModerationActions by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(ModerationActionWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(ModerationActionWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as ModerationActionWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is ModerationActionWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(ModerationActionOrderByInput orderBy) {

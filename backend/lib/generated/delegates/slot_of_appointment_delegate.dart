...
class SlotOfAppointmentDelegate {
  ...
  SlotOfAppointmentDelegate(this._executor);

  /// Find a single SlotOfAppointment by unique field(s)
  Future<SlotOfAppointment?> findUnique({
    required SlotOfAppointmentWhereUniqueInput where,
  }) async {
    ...
    return result != null ? SlotOfAppointment.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single SlotOfAppointment or throw if not found
  Future<SlotOfAppointment> findUniqueOrThrow({
    required SlotOfAppointmentWhereUniqueInput where,
  }) async {
    final result = await findUnique(where: where);
    if (result == null) {
      throw Exception('SlotOfAppointment not found');
    }
    return result;
  }

  /// Find the first SlotOfAppointment matching criteria
  Future<SlotOfAppointment?> findFirst({
    SlotOfAppointmentWhereInput? where,
    SlotOfAppointmentOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple SlotOfAppointments with optional filters
  Future<List<SlotOfAppointment>> findMany({
    SlotOfAppointmentWhereInput? where,
    SlotOfAppointmentOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => SlotOfAppointment.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new SlotOfAppointment
  Future<SlotOfAppointment> create({
    required CreateSlotOfAppointmentInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('SlotOfAppointment')
    ...
    final result = await _executor.executeQueryAsSingleMap(query);
    if (result == null) {
      throw Exception('Failed to create SlotOfAppointment');
    }
    return SlotOfAppointment.fromJson(_normalizeForJson(result));
  }

  /// Create multiple SlotOfAppointments
  Future<int> createMany({
    required List<CreateSlotOfAppointmentInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('SlotOfAppointment')
  ...
  /// Update a SlotOfAppointment
  Future<SlotOfAppointment> update({
    required SlotOfAppointmentWhereUniqueInput where,
    required UpdateSlotOfAppointmentInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('SlotOfAppointment')
  ...
  /// Update multiple SlotOfAppointments
  Future<int> updateMany({
    required SlotOfAppointmentWhereInput where,
    required UpdateSlotOfAppointmentInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('SlotOfAppointment')
  ...
  /// Delete a SlotOfAppointment
  Future<SlotOfAppointment> delete({
    required SlotOfAppointmentWhereUniqueInput where,
  }) async {
  ...
  /// Delete multiple SlotOfAppointments
  ...
  /// Count SlotOfAppointments matching criteria
  Future<int> count({
    SlotOfAppointmentWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group SlotOfAppointments by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(SlotOfAppointmentWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(SlotOfAppointmentWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as SlotOfAppointmentWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is SlotOfAppointmentWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(SlotOfAppointmentOrderByInput orderBy) {

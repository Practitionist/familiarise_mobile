...
class PlanMaterialDelegate {
  final BaseExecutor _executor;

  PlanMaterialDelegate(this._executor);

  /// Find a single PlanMaterial by unique field(s)
  Future<PlanMaterial?> findUnique({
    required PlanMaterialWhereUniqueInput where,
  }) async {
    ...
    return result != null ? PlanMaterial.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single PlanMaterial or throw if not found
  Future<PlanMaterial> findUniqueOrThrow({
    required PlanMaterialWhereUniqueInput where,
  }) async {
    final result = await findUnique(where: where);
    if (result == null) {
      throw Exception('PlanMaterial not found');
    }
    return result;
  }

  /// Find the first PlanMaterial matching criteria
  Future<PlanMaterial?> findFirst({
    PlanMaterialWhereInput? where,
    PlanMaterialOrderByInput? orderBy,
  }) async {
  ...
  /// Find multiple PlanMaterials with optional filters
  Future<List<PlanMaterial>> findMany({
    PlanMaterialWhereInput? where,
    PlanMaterialOrderByInput? orderBy,
    int? take,
    int? skip,
    ...
    return results.map((json) => PlanMaterial.fromJson(_normalizeForJson(json))).toList();
  }

  /// Create a new PlanMaterial
  Future<PlanMaterial> create({
    required CreatePlanMaterialInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('PlanMaterial')
    ...
    final result = await _executor.executeQueryAsSingleMap(query);
    if (result == null) {
      throw Exception('Failed to create PlanMaterial');
    }
    return PlanMaterial.fromJson(_normalizeForJson(result));
  }

  /// Create multiple PlanMaterials
  Future<int> createMany({
    required List<CreatePlanMaterialInput> data,
  }) async {
    final query = JsonQueryBuilder()
        .model('PlanMaterial')
  ...
  /// Update a PlanMaterial
  Future<PlanMaterial> update({
    required PlanMaterialWhereUniqueInput where,
    required UpdatePlanMaterialInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('PlanMaterial')
  ...
  /// Update multiple PlanMaterials
  Future<int> updateMany({
    required PlanMaterialWhereInput where,
    required UpdatePlanMaterialInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('PlanMaterial')
  ...
  /// Delete a PlanMaterial
  Future<PlanMaterial> delete({
    required PlanMaterialWhereUniqueInput where,
  }) async {
  ...
  /// Count PlanMaterials matching criteria
  Future<int> count({
    PlanMaterialWhereInput? where,
  }) async {
    final queryBuilder = JsonQueryBuilder()
  ...
  /// Group PlanMaterials by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(PlanMaterialWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(PlanMaterialWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as PlanMaterialWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is PlanMaterialWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(PlanMaterialOrderByInput orderBy) {
